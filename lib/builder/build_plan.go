...
type BuildPlan struct {
	...
	stages        []*buildStage
	pseudoStages  map[string]*buildStage
	allowModifyFS bool
	forceCommit   bool
}
...
func NewBuildPlan(
	...
	plan := &BuildPlan{
		...
		pseudoStages:  make(map[string]*buildStage),
	...
	for i, parsedStage := range parsedStages {
		...
		for alias, dirs := range stage.crossRefDirs {
			...
			if utils.IsImageName(alias) {
				if err := plan.addPseudoStage(alias, digestPairs); err != nil {
					return nil, fmt.Errorf("add pseudo stage: %v", err)
				}
				aliases[alias] = true
			}
...
func (plan *BuildPlan) Execute() (*image.DistributionManifest, error) {
	...
	for imageName, stage := range plan.pseudoStages {
		...
		if err := stage.build(plan.cacheMgr, false, true); err != nil {
			return nil, fmt.Errorf("build pseudo stage: %s", err)
		}
	}
	...
	for k := 0; k < len(plan.stages); k++ {
		...
		if plan.allowModifyFS {
			if k < len(plan.stages)-1 {
				...
				newRoot := currStage.ctx.CrossRefDir(currStage.alias)
...
func (plan *BuildPlan) addPseudoStage(imageName string, digestPairs map[string][]*image.DigestPair) error {
	if _, found := plan.pseudoStages[imageName]; found {
		return nil
	}
	fakeStage := &dockerfile.Stage{
		From: &dockerfile.FromDirective{
			Image: imageName,
			Alias: imageName,
		},
	}
	fakeBuildStage, err := newBuildStage(plan.baseCtx, fakeStage, digestPairs, false, false)
	if err != nil {
		return fmt.Errorf("failed to create pseudo build stage for image name %s: %v", imageName, err)
	}
	plan.pseudoStages[imageName] = fakeBuildStage
	return nil
}
